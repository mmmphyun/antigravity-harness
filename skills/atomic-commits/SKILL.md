...
### 4단계: Green-State 즉시 로컬 커밋 & 푸시 시점 분리
...
- **로컬 즉시 커밋**: 단위 테스트가 통과하면 다음 태스크 코드를 작성하기 전에 **즉시 터미널에서 로컬 커밋을 실행**합니다.
  ```powershell
  git commit -m "feat(auth): JWT 토큰 검증 로직 구현"
  ```
- **즉각 푸시 금지 (No Push per Commit)**:
  - 매 커밋마다 `git push`를 때리는 행위를 금지합니다 (CI 리소스 고갈 및 네트워크 턴 병목 차단).
- **원격 푸시 시점 (Batch/Milestone End Push)**:
  - 분해된 모든 하위 태스크의 커밋이 로컬에 완료되고, **전체 배치 검증이 완료된 시점에 현재 활성 브랜치로 1회 푸시**합니다.
  - **예외 허용**: 작업 세션이 종료되거나 사용자가 명시적으로 푸시를 요청한 경우에만 중간 푸시를 허용합니다.
  ```powershell
  # 현재 활성 브랜치로 안전 푸시
...
2. **워킹 트리 오염(Dirty Tree) 상태에서 푸시 하드 차단 (`deny`)**:
   - 커밋되지 않은 로컬 수정/삭제 파일이 남아있는 상태에서 `git push` 실행 시 즉시 실행 거부.
3. **스테이징 파일 상한선 및 승인 요구 (`ask`)**:
   - 단일 커밋당 스테이징 파일 수가 **4개를 초과**할 경우, 에이전트 자의적 커밋을 차단하고 사용자에게 승인(`ask`)을 요구.
4. **대량 변경 예외 허용 (Bulk Exemptions)**:
...
## 3. Git 브랜치 및 머지 전략 연계

- **작업 브랜치**: 개발 중에는 장애 격리와 빠른 롤백을 위해 철저히 1-Task 1-Commit 원칙을 준수하여 로컬 마이크로 커밋 히스토리를 유지합니다.
- **병합 전략**:
  - 1인 개발/단독 작업: `main` 브랜치에 직접 마이크로 커밋들을 안전하게 푸시합니다.
  - 팀/PR 기반 작업: PR을 `main`에 병합할 때 **Squash and Merge**를 사용하여 기능 단위 단일 원자적 커밋으로 축약 병합할 수 있습니다.
