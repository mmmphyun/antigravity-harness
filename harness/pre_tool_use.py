...
COMMIT_REGEX = re.compile(r'^(feat|fix|refactor|docs|chore|test|style|perf|build|ci)(\([a-zA-Z0-9_\-\./]+\))?:\s+(.+)$')
...
def check_infra_mutation(command_line: str) -> tuple[bool, str]:
    ...
    if cmd_lower.startswith("docker ") or cmd_lower.startswith("docker.exe "):
        if any(action in cmd_lower for action in [" rm ", " rmi ", " stop ", " kill ", " system prune", " volume rm"]):
...
def main():
    try:
            ...
            if re.search(r'\bgit\s+commit\b', cmd_line):
                ...
                if commit_msg:
                    ...
                    if not match or not has_korean:
                        ...
                        state_manager.log_event("COMMIT_CONVENTION_BLOCKED", conv_id, "비규격 커밋 메시지 차단", first_line)
                        print(json.dumps({"decision": "deny", "reason": reason}, ensure_ascii=False))
                        return
        ...
        try:
            state_manager.log_event("PRE_TOOL_USE_ERROR", "system", f"Fail-open triggered: {str(e)}")
        except Exception:
            pass
        print(json.dumps({}, ensure_ascii=False))
